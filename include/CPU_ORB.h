...
namespace MORB
{
	...
	public:
		...
		static void MatchPic(ORB_CPU &orb1, ORB_CPU &orb2, bool cross = false, bool matchfilter = false);	//注意两点，第一static关键词只需要在声明时指明即可；第二由于静态方法无法直接调用一般成员，这里只有通过对象引用来实现（因为静态方法出现于一般成员之前，所以这里不能定义形参，只有引用）
